...
This analysis is conducted under several assumptions and has a number of limitations that should be acknowledged.

First, the analysis assumes that the Bank Marketing dataset is representative of future marketing campaigns. However, the data were collected during a specific time period and economic context, which may limit the generalizability of the results.

Second, the target variable is highly imbalanced, with only around 11–12% of clients subscribing to a term deposit. While this was addressed by using stratified train-test splits and evaluation metrics such as F1-score and ROC-AUC, no explicit resampling techniques were applied. As a result, model performance on the minority class may still be limited.

Third, this project focuses on a baseline logistic regression model with limited hyperparameter tuning. More complex models or additional feature engineering may further improve predictive performance.

Finally, all conclusions are based solely on the available features in the dataset. Any unobserved factors influencing client subscription behavior are not captured and may affect the results.
